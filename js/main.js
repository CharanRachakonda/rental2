...
function addVehicleToCart(model, price) {
    ...
    const rentalDetails = { model, price, days, date: new Date().toLocaleDateString() };
    const currentUserEmail = JSON.parse(localStorage.getItem('loggedInUser'));
    let rentalHistory = JSON.parse(localStorage.getItem(`${currentUserEmail}_rentalHistory`)) || [];
    rentalHistory.push(rentalDetails);
    localStorage.setItem(`${currentUserEmail}_rentalHistory`, JSON.stringify(rentalHistory));
    // alert("Vehicle rented out successfully!");
    selectedVehicle = { model, price, days };
